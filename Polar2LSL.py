# Original from Mark Span - https://github.com/markspan/PolarBand2lsl
# Adapted to specific Polar H10 band
...
def data_conv(sender, data: bytearray):
    ...
    if data[0] == 0x00:
        ...
        while offset < len(samples):
            ...
            if not running:
                running = True
                print(f"\nUnix Timestamp = {time.time() * 1000}, Local Timestamp = {local_clock()}")
                print("\nStream started...")
...
async def run(client, debug=False):
    ...
    await aioconsole.ainput('Press a key to quit')
